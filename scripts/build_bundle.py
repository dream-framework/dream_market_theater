...
def build_theater_trade_signals(coupling_rows: list[dict[str, Any]], live_predictions: list[dict[str, Any]], max_per_topic: int = 8) -> list[dict[str, Any]]:
    """Attach real live prediction rows to theater topics for tooltip context.

    This does not score or trade. It mirrors the app's research coupling logic:
    topic-level retained cycle pressure is paired with the scored non-h1 horizon,
    then the tooltip shows the live rows that would be inspected under that
    horizon. No dummy ticker rows are created.
    """
    ...
    for c in sorted(usable_couplings, key=lambda r: float(r.get("coupling_score") or 0.0), reverse=True):
        ...
        seen.add((topic, h))
        signals = []
        for row in by_horizon.get(h, [])[:max_per_topic]:
            signals.append({
        ...
        out.append({
            ...
            "status": c.get("status"),
            "signals": signals,
            "source_policy": "real-live-predictions-only",
        })
    return out
